/* ============================================================
   GameScene.js v0.3 FIX
   핵심 수정: 맵 20×20 (250×250은 Three.js 오브젝트 125,000+개
   동기 생성으로 메인스레드 블록 → 게임 화면 미표시)
   카메라/포그 수치 20×20 기준 재조정
   _makeTextSprite 중복 선언 제거 (GridMap.js 전역 사용)
   OrbitControls 없어도 크래시 없도록 null 안전 처리
   ============================================================ */
...
function _generateMap() {
  ...
  for(let r=0;r<rows;r++){
    ...
    for(let c=0;c<cols;c++){
      ...
      else if(r===Math.floor(rows*0.4)||r===Math.floor(rows*0.6)){t='VALLEY';}
      else{const rnd=Math.random();if(rnd<0.18)t='FOREST';else if(rnd<0.28)t='HILL';else t='OPEN';}
      row.push(t);
    }
    map.push(row);
  }
...
function _calcObjective(){
  return{col:Math.floor(CONFIG.GRID_COLS/2),row:Math.floor(CONFIG.GRID_ROWS/2)};
}
...
class GameScene{
  ...
  _initScene(){
    ...
    this.scene3d.fog=new THREE.FogExp2(0x040604,0.04);
    ...
    try{
      if(typeof THREE.OrbitControls!=='undefined'){
        ...
        Object.assign(this.controls,{
          ...
          maxPolarAngle:Math.PI/2.05,screenSpacePanning:true,
        });
      }
    }catch(e){console.warn('[GameScene] OrbitControls 없음:',e.message);}

    this.scene3d.add(new THREE.AmbientLight(0x0a2010,1.2));
    const pl=new THREE.PointLight(0x39ff8e,1.2,80);
    pl.position.set(0,30,0);this.scene3d.add(pl);
    const pl2=new THREE.PointLight(0x2277cc,0.4,60);
    pl2.position.set(-10,20,10);this.scene3d.add(pl2);
  ...
  _drawObjective(){
    ...
    const S=0.6;
    const pts=[new THREE.Vector3(x-S,tH+0.04,z-S),new THREE.Vector3(x+S,tH+0.04,z-S),
               new THREE.Vector3(x+S,tH+0.04,z+S),new THREE.Vector3(x-S,tH+0.04,z+S),
    ...
    const star=_makeTextSprite('★','#ffb84d');star.position.set(x,tH+1.2,z);star.scale.set(2,2,1);this.scene3d.add(star);
    const lbl=_makeTextSprite('OBJ','#ffb84d');lbl.position.set(x,tH+0.6,z);lbl.scale.set(1.2,0.7,1);this.scene3d.add(lbl);
  }
  ...
  _makeSquad(id,side,col,row){return{id,side,pos:{col,row},troops:CONFIG.SQUAD_TROOP_MAX,ap:CONFIG.SQUAD_AP_MAX,terrain:CONFIG.TERRAIN.OPEN,alive:true,mesh:null,mat:null,boxMesh:null};}
  ...
  _createMesh(squad){
    ...
    const box=new THREE.Mesh(geo,mat);box.userData={squadId:squad.id};group.add(box);
    const el=new THREE.LineSegments(new THREE.EdgesGeometry(geo),new THREE.LineBasicMaterial({color:cd.hex,transparent:true,opacity:0.9}));
    el.raycast=()=>{};group.add(el);
    const sprite=_makeSquadLabelSprite(label,cd.css,cd.bg);
    sprite.position.set(0,1.2,0);sprite.scale.set(2.0,0.9,1);sprite.raycast=()=>{};group.add(sprite);
    group.position.set(x,y,z);group.userData={squadId:squad.id};
    this.scene3d.add(group);squad.mesh=group;squad.mat=mat;squad.boxMesh=box;squad._colDef=cd;
  }
  ...
  _updateOverlapVisuals(){
    ...
    this._overlapBadges={};
    const groups={};
    for(const s of this.squads.filter(q=>q.alive&&q.mesh)){const key=`${s.pos.col},${s.pos.row}`;(groups[key]=groups[key]||[]).push(s);}
    for(const[key,list]of Object.entries(groups)){
      const[col,row]=key.split(',').map(Number);const base=this.gridMap.toWorld(col,row);
      if(list.length===1){list[0].mesh.position.set(base.x,base.y,base.z);}
        ...
        badge.position.set(base.x,base.y+2.0,base.z);badge.scale.set(1.2,0.7,1);badge.renderOrder=10;
  ...
  _showSquadPicker(squads,clientX,clientY){
    ...
    squads.forEach(squad=>{
      const cd=squad._colDef||this._squadColor(squad);const q=this._quality(squad);
    ...
    const closeBtn=document.createElement('div');closeBtn.className='picker-close';closeBtn.innerHTML='✕&nbsp;&nbsp;닫기';
    closeBtn.addEventListener('click',e=>{e.stopPropagation();this._hideSquadPicker();});picker.appendChild(closeBtn);
    const rect=this.container.getBoundingClientRect();const PW=224,PH=48+squads.length*76+38;
  ...
  selectSquad(squad){
    ...
    this.selectedSquad=squad;const cd=squad._colDef||this._squadColor(squad);
    ...
    if(this.comms.rollMishear(quality)){
      ...
      if(res.distorted){
        ...
        if(res.mishearType==='coord'){
          ...
          if(dd>0&&dc<=squad.ap&&dd<=squad.ap){this.pendingCmds.push({type:'move',squadId:squad.id,targetPos:dp});squad.ap-=dc;this.gridMap.clearHighlights();this.gridMap.highlightTile(dp.col,dp.row,0xffb84d,0.50);chatUI.addLog(`A${squad.id}`,null,`⚡ 오청 이동 → ${String.fromCharCode(65+(dp.col%26))}-${String(dp.row+1).padStart(2,'0')}`);}
          else{chatUI.addLog(`A${squad.id}`,null,'⚡ 오청 좌표 이동 불가 → 대기','system');}
          this._clearSelection();return;
        }
      }
    }
    this.pendingCmds.push({type:'move',squadId:squad.id,targetPos});squad.ap-=cost;
  ...
  _issueAttack(squad,target){
    ...
    this.pendingCmds.push({type:'attack',squadId:squad.id,targetId:target.id});squad.ap-=1;
    this.gridMap.clearHighlights();this.gridMap.highlightTile(target.pos.col,target.pos.row,0xff4444,0.50);
    chatUI.addLog(`A${squad.id}`,null,`E${target.id-CONFIG.SQUAD_COUNT}분대 사격 명령`);this._clearSelection();
  }
  ...
  moveSquadTo(squad,targetPos,onDone){
    ...
    this._animations.push({type:'move',squad,from,to,duration:0.32,elapsed:0,onComplete:()=>{squad.pos={...targetPos};squad.terrain=this.gridMap.tiles[targetPos.row][targetPos.col].terrain;squad.mesh.position.set(to.x,to.y,to.z);this._updateOverlapVisuals();if(onDone)onDone();}});
  }
  ...
  _syncPanel(){
    ...
    document.querySelectorAll('.squad-card').forEach(card=>{
      const sid=parseInt(card.dataset.squadId);const squad=allies.find(s=>s.id===sid);if(!squad)return;
      ...
      const cd=squad._colDef||this._squadColor(squad);card.style.borderLeft=`3px solid ${cd.css}`;
  ...
  _setupInput(){
    ...
    cv.addEventListener('pointerup',e=>{
      ...
      this._mouseDownPos=null;if(dx<6&&dy<6)this._onCanvasClick(e);
    });
  ...
  _onCanvasClick(e){
    ...
    if(hits.length>0){
      const{squadId}=hits[0].object.userData;const clicked=this.squads.find(s=>s.id===squadId&&s.alive);
      if(clicked){
    ...
    const tHits=this.raycaster.intersectObjects(this.gridMap.getTileMeshes());if(tHits.length===0)return;
  ...
  _raycastTile(e){this.raycaster.setFromCamera(this._toNDC(e),this.camera);const h=this.raycaster.intersectObjects(this.gridMap.getTileMeshes());return h.length?h[0].object.userData:null;}
  ...
  _updateAnimations(delta){
    ...
    for(const a of this._animations){
      a.elapsed+=delta;const t=Math.min(a.elapsed/a.duration,1);const e=t<0.5?2*t*t:-1+(4-2*t)*t;
      if(a.type==='move'){a.squad.mesh.position.x=a.from.x+(a.to.x-a.from.x)*e;a.squad.mesh.position.z=a.from.z+(a.to.z-a.from.z)*e;a.squad.mesh.position.y=a.from.y+(a.to.y-a.from.y)*e+Math.sin(Math.PI*t)*0.8;}
  ...
  _tick(ts){
    ...
    const now=ts||0,delta=Math.min((now-(this._lastTime||now))/1000,0.1);this._lastTime=now;
    this._updateAnimations(delta);if(this.controls)this.controls.update();
