...
class GameScene {
  ...
  _focusOnAllyCenter() {
    ...
    const avgCol = allies.reduce((s,q) => s + q.pos.col, 0) / allies.length;
    const avgRow = allies.reduce((s,q) => s + q.pos.row, 0) / allies.length;
    const gm = this.gridMap;
    const wx = avgCol * gm.TILE_W + gm.OFFSET_X;
    const wz = avgRow * gm.TILE_W + gm.OFFSET_Z;
  ...
  _animateCamera(tx, ty, tz) {
    ...
    this._camTweenFn = (delta) => {
      ...
      elapsed += delta;
      const t = Math.min(elapsed / duration, 1);
      this.controls.target.lerpVectors(startTarget, endTarget, ease(t));
      this.controls.update();
      if (t >= 1) { this._camTweenFn = null; this._camTweenCancel = null; }
    };
  }
